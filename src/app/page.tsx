...
interface DishInCart extends Dish {
  quantity: number;
}
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchRandomDishes = async () => {
      try {
        ...
        const dishes = results.map(r => r.meals[0]);
  ...
  const generateRandomPrice = () => `${Math.floor(Math.random() * 30) + 35} ر.س`;
  ...
  const handleAddToCart = (dish: Dish) => {
    ...
    const cart: DishInCart[] = cartStr ? JSON.parse(cartStr) : [];

    const existingIndex = cart.findIndex(item => item.idMeal === dish.idMeal);
    if (existingIndex >= 0) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative h-screen flex items-center justify-center">
        ...
        <div className="relative z-10 text-center text-white px-4">
          ...
          <p className="text-xl md:text-2xl mb-8 max-w-2xl mx-auto">
            استمتع بأشهى المأكولات المحضرة بأيدي أمهر الطهاة باستخدام أفضل المكونات الطازجة
          </p>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* Feature Boxes */}
            {[
              { title: "جودة عالية", desc: "نستخدم أفضل المكونات الطازجة في جميع أطباقنا." },
              { title: "خدمة سريعة", desc: "توصيل الطلبات بسرعة وجودة عالية." },
              { title: "تنوع المأكولات", desc: "قائمة طعام متنوعة ترضي جميع الأذواق." }
            ].map((f, idx) => (
      ...
      <section className="py-16 bg-amber-50">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12">أطباق مميزة</h2>
          {loading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
              {Array.from({ length: 4 }).map((_, idx) => (
                <div key={idx} className="bg-gray-200 h-64 rounded-lg animate-pulse" />
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
            {featuredDishes.map((dish) => (
              <div key={dish.idMeal} className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col">
                <div className="relative h-48">
                  <Image src={dish.strMealThumb} alt={dish.strMeal} fill className="object-cover" sizes="(max-width: 768px) 100vw, 33vw" />
                </div>
                <div className="p-4 flex-grow flex flex-col">
                  <h3 className="text-lg font-semibold mb-2">{dish.strMeal}</h3>
                  <p className="text-sm text-gray-600 flex-grow">{dish.strInstructions.slice(0, 50)}...</p>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map(({ id, name, comment, rating, image }) => (
              <div key={id} className="bg-amber-50 rounded-lg p-6 shadow-md flex flex-col items-center text-center">
                <Image src={image} alt={name} width={80} height={80} className="rounded-full mb-4 object-cover" />
                <h3 className="font-semibold text-lg mb-2">{name}</h3>
                <p className="text-gray-700 mb-3">"{comment}"</p>
                <div className="flex space-x-1 justify-center">
                  {Array.from({ length: rating }).map((_, idx) => (
                    <svg key={idx} xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-amber-600" fill="currentColor" viewBox="0 0 20 20">
